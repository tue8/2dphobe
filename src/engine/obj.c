...
void obj_create(obj_t *obj_p, renderer_t *renderer_p, vec3 v_pos, vec3 v_scale, unsigned int tex_id)
{
	...
	glm_translate(obj_p->local_mat, BREAK_VEC3(v_pos));
	glm_scale(obj_p->local_mat, BREAK_VEC3(v_scale));
	obj_p->tex_id = tex_id;
}
...
void obj_draw(obj_t *obj_p, renderer_t *renderer_p)
{
	...
	renderer_draw_quad(renderer_p, obj_p->tex_id, obj_p->local_mat_index);
}
...
{
	glm_rotate(obj_p->local_mat, angle, BREAK_VEC3(v_axis));
}

void obj_get_scale(obj_t *obj_p, renderer_t *renderer_p, vec3 v_scale)
{
	mat4 rotation;
	glm_decompose_rs(obj_p->local_mat, rotation, v_scale);
}
