...
void obj_create(obj_t *obj_p, renderer_t *renderer_p, phobe_vec3 pos,
...
void obj_get_pos(obj_t *obj_p, renderer_t *renderer_p, phobe_vec3 pos)
{
	vec4 pos_v4;
...
void obj_set_pos(obj_t *obj_p, renderer_t *renderer_p, phobe_vec3 pos)
{
	vec3 v_pos = {pos.x, pos.y, pos.z};
	COPY_VEC3(obj_p->local_mat[3], v_pos);
}

void obj_add_pos(obj_t *obj_p, renderer_t *renderer_p, phobe_vec3 pos)
{
	glm_translate(obj_p->local_mat, RAW_VEC3(pos));
}

void obj_add_rot(obj_t *obj_p, renderer_t *renderer_p, float angle, phobe_vec3 axis)
{
	glm_rotate(obj_p->local_mat, angle, RAW_VEC3(axis));
}

void obj_get_scale(obj_t *obj_p, renderer_t *renderer_p, phobe_vec3 scale)
{
	mat4 rotation;
	glm_decompose_rs(obj_p->local_mat, rotation, RAW_VEC3(scale));
}

void obj_add_scale(obj_t *obj_p, renderer_t *renderer_p, phobe_vec3 scale)
{
	glm_scale(obj_p->local_mat, RAW_VEC3(scale));
}